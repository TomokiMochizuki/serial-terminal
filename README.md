...
- 1接続 = 1タブ。複数の接続を1アプリで同時に操作可能
- 接続種別をタブ内のコンボで選択 (シリアル / TCPクライアント / TCPサーバ / UDP)。
  種別に応じてパラメータ欄が切り替わります
  - シリアル: ポート / ボーレート / データビット / パリティ / ストップビット
  - TCPクライアント: 接続先 Host / Port
  - TCPサーバ: 待受 Port。1クライアントのみ受け入れ (接続中の新規接続は拒否)、
    クライアント切断後は自動で再待受に戻ります
  - UDP: 宛先 Host:Port と受信 Port (コネクションレス送受信)
  - 各種別のネットワーク設定は接続成功時に保存され、新規タブの既定値になります
